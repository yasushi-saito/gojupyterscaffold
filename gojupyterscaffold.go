...
func (s *Server) monitorSigint() {
	...
	go func() {
		for range ch {
			log.Print("Received SIGINT. Cancelling an ongoing execute_request")
...
func (s *Server) Loop() {
	go func() {
		log.Print("Forwarding heartbeat requests")
		if err := zmq.Proxy(s.hb, s.hb, nil); err != nil {
			log.Panicf("Failed to echo heartbeat request: %v", err)
		}
		log.Print("Quitting goroutine for heartbeat requests")
	}()
	s.monitorSigint()
